...
def _within_days(published_date: str, today: str, max_age_days: int) -> bool:
    ...
    unverifiable rather than stale — not every provider/result reliably
    attaches a publish date, and dropping every undated result starves the
    ...
    (INA_DATETIME_OVERRIDE), where a provider may otherwise return articles
...
def build_web_search_node(providers: list[WebSearchProvider], search_workers: int):
    def web_search_node(state: AgentState) -> AgentState:
        today = today_utc()
        provider = providers[state["search_provider_index"]]
        ...
        # and widen the recency window on retry, the same way a domain
        # filter (where the provider has one) already opens up on retry.
        ...
        with ThreadPoolExecutor(max_workers=search_workers) as pool:
            futures = {pool.submit(provider.search, q, time_range): q for q in state["queries"]}
            for future in as_completed(futures):
                query = futures[future]
                try:
                    result = future.result()
                    if "error" in result:
                        # Providers (e.g. TavilySearch.invoke()) may return
                        # {"error": ValueError(...)} instead of raising on
                        # API-side failures (quota limits, auth, outages) —
                        # treat that the same as an exception so it's
                        # visible, rather than silently reading as "0
                        # results found" (indistinguishable from a quiet
                        # news day) via the `.get("results", [])` fallback
                        # below.
                        error_count += 1
                        print(
                            f"[agent] Web search ({provider.name}): query {query!r} failed — {result['error']!r}"
                        )
                        continue
                    ...
                    error_count += 1
                    print(
                        f"[agent] Web search ({provider.name}): query {query!r} failed — {type(exc).__name__}: {exc}"
                    )
        if future_count:
            print(
                f"[agent] Web search ({provider.name}): dropped {future_count} result(s) dated after {today} (future — backtesting guard)"
            )
        if stale_count:
            print(
                f"[agent] Web search ({provider.name}): dropped {stale_count} result(s) older than {max_age_days}d"
            )
        print(
            f"[agent] Web search ({provider.name}): {len(new_results)} result(s) kept"
            + (
                f", {error_count}/{len(state['queries'])} quer{'y' if error_count == 1 else 'ies'} failed"
                if error_count
                else ""
            )
        )
