...
def build_news_snippet_getter_node(s3_client, input_bucket: str):
    def news_snippet_getter_node(state: AgentState) -> AgentState:
        today = today_utc()
        prefix = f"input/{today}/"
        ...
        snippets = [
            s3_client.get_object(Bucket=input_bucket, Key=key)["Body"]
            .read()
            .decode("utf-8")
            .strip()
            for key in keys
