...
# The model is only ever asked to start its output at "## 1. Executive
# Summary" (see market_analyst.j2's OUTPUT FORMAT), but sometimes prepends
# its own top-level title line first — e.g. "# Investment Intelligence
# Report: <date>" — duplicating the hardcoded title added below. `#(?!#)`
# matches only single-`#` heading lines, so the real `##` section headings
# are never touched.
...
def build_market_analyst_node(llm, system_prompt: str):
    def market_analyst_node(state: AgentState) -> AgentState:
        ...
        body = _LEADING_H1_RE.sub("", response.content.strip())
        report_md = (
